...
namespace test.car
{
    internal class CarManager
    {
        ...
        // 查看某辆车 方法
        //需要外部给数据，车辆编号，括号里要写int ID，这里是形参
        public void SearchOne(int id)
        {
            ...
            // 判断文件是否存在===存在，读取文件，反序列化 ===》根据id查找车辆对象===》找不到则提示===》找到了就输出（硬盘文件是字符串，程序内存是列表/对象
            string jsonStr = File.ReadAllText(this.Path);
            List<Car> cars = JsonSerializer.Deserialize<List<Car>>(jsonStr);
            // 使用列表的Find 实现查找carobj是cars列表里的单个car对象
